// ─────────────────────────────────────────────────────────────────────────────
//  RECOMMENDED HOMEPAGE — the synthesised direction, as a working mockup.
//  Light theme matching SkyRa's brand, with a live savings calculator and a
//  residential/commercial packages toggle.
// ─────────────────────────────────────────────────────────────────────────────
import { useState } from 'react'
import {
  ArrowRight, Phone, Check, Zap, ShieldCheck, Sun, BatteryCharging, Gauge,
  ClipboardCheck, PencilRuler, Wrench, Power, Star, MapPin,
} from 'lucide-react'
import { SunOrb, SectionHeading } from './ui'

const HERO_IMG =
  'https://images.unsplash.com/photo-1508514177221-188b1cf16e9d?q=80&w=2670&auto=format&fit=crop'

/* Browser chrome wrapper so the page reads as a framed mockup */
function SiteFrame({ children }) {
  return (
    <div className="overflow-hidden rounded-2xl border border-[#24304A] shadow-[0_40px_120px_-40px_rgba(0,0,0,0.8)]">
      <div className="flex items-center gap-3 border-b border-[#1E2A42] bg-[#0B1422] px-4 py-2.5">
        ...
        <div className="flex flex-1 items-center gap-2 rounded-md border border-[#1E2A42] bg-[#0E1726] px-3 py-1">
          <span className="h-1.5 w-1.5 rounded-full bg-[#38BDF8]" />
          <span className="text-[11px] font-medium text-[#6B7C99]">skyra.com.au</span>
        </div>
        <span className="rounded-full bg-[#38BDF8]/15 px-2.5 py-0.5 text-[10px] font-bold uppercase tracking-wider text-[#38BDF8]">
          Recommended
        </span>
      </div>
      ...
      <div className="relative z-10 mx-auto flex max-w-5xl flex-col px-5 pb-12 pt-32 sm:px-8">
        <div className="grid items-end gap-8 lg:grid-cols-[1.6fr_1fr]">
          <div>
            ...
            <div className="font-fraunces text-[clamp(40px,6.4vw,76px)] font-black italic leading-[0.92] text-[#7DD3FC]">
              Save up to 70%.
            </div>
        ...
        <div className="grid items-center gap-10 lg:grid-cols-2">
          <div>
            ...
            <h2 className="mt-2 text-[clamp(28px,4vw,40px)] font-extrabold leading-tight tracking-tight text-slate-900">
              See your saving in
              <span className="font-fraunces italic text-sky-600"> seconds.</span>
            </h2>
    ...
    <section className="bg-white px-5 py-16 sm:px-8">
      <div className="mx-auto max-w-5xl text-center">
        ...
        <h2 className="mt-2 text-[clamp(28px,4vw,42px)] font-extrabold tracking-tight text-slate-900">
          Premium solar <span className="font-fraunces italic text-sky-600">packages</span>
        </h2>
          ...
          {steps.map((s) => (
            <div key={s.n} className="rounded-2xl border border-white/10 bg-white/[0.04] p-6">
              <div className="flex items-center justify-between">
                ...
                <span className="font-fraunces text-[34px] font-black italic text-white/10">{s.n}</span>
              </div>
      ...
      <div className="mx-auto max-w-5xl">
        <div className="flex flex-wrap items-end justify-between gap-4">
          <h2 className="max-w-md text-[clamp(26px,3.6vw,38px)] font-extrabold tracking-tight text-slate-900">
            Loved by homeowners <span className="font-fraunces italic text-sky-600">Australia-wide.</span>
          </h2>
      ...
      <div className="relative mx-auto grid max-w-5xl items-center gap-10 lg:grid-cols-[1.1fr_1fr]">
        <div>
          <h2 className="text-[clamp(30px,4.4vw,48px)] font-extrabold leading-[1.02] tracking-tight text-white">
            Ready to cut your
            <br />
            power bill? <span className="font-fraunces italic text-[#7DD3FC]">Let's go.</span>
          </h2>
          ...
          <div className="mt-6 flex flex-wrap gap-5">
            {[['ShieldCheck', 'CEC accredited'], ['Check', 'No-obligation'], ['Zap', '25-yr warranty']].map(([, l]) => (
  ...
  return (
    <div className="mx-auto max-w-[1200px] px-5 py-10 sm:px-8">
      <SectionHeading
        ...
        title="A homepage that sells the saving"
        sub="The strongest patterns from the research, synthesised into your brand: a savings-led hero, an interactive calculator, packages with a residential/commercial toggle, a clear four-step process and a low-friction quote form. The calculator and toggle below are live — try them."
      />
      <div className="mt-9">
        <SiteFrame>
          <Hero />
